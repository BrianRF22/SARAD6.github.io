...
$datos = pg_fetch_all(pg_query($conexion, "SELECT pa.*, p.per_ci, p.per_nombre, p.per_apellido, to_char(p.fecha_nacimiento, 'dd/mm/yyyy') fecha_nacimiento_formato, p.celular FROM padres pa, personas p WHERE p.id_per = pa.id_per ORDER BY pa.id_padre"));
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    <main>
        <div>
            <h5 class="display-2 fw-bold">Padres 
            ...
            <div class="table-responsive">
                <table class="table text-white">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <button class="btn input-group-text text-success" type="button" onclick="modificar('<?php echo $d['id_padre']; ?>', '<?php echo $d['per_ci']; ?>', '<?php echo $d['per_nombre']; ?>', '<?php echo $d['per_apellido']; ?>', '<?php echo $d['fecha_nacimiento_formato']; ?>', '<?php echo $d['celular']; ?>');">
                                    <i class="fa fa-pencil-square-o"></i>
                                </button>
                            </td>
                            <td><?php echo $d['id_padre']; ?></td>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5 text-primary">Agregar Padre</h1>
                    ...
                    <div class="modal-body">
                        ...
                        <label class="text-success">Adjuntar Archivo</label>
                        <div>
                            <input class="form-control" type="file" id="archivo_imagen" name="archivo_imagen" accept="image/png, image/jpeg, image/jpg">
                        </div>
                    </div>
    ...
    <div class="modal fade" id="modal-modificar" tabindex="-1" aria-labelledby="modal-modificar" aria-hidden="true">
        <form action="modificar.php" method="POST" enctype="multipart/form-data">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5 text-primary">Modificar Padre</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <label class="text-success">Adjuntar Archivo</label>
                        <div>
                            <input class="form-control" type="file" id="mod_archivo_imagen" name="archivo_imagen" accept="image/png, image/jpeg, image/jpg">
                        </div>
                    </div>
                    <div class="modal-footer justify-content-between">
                        <button type="submit" class="btn btn-success"><i class="fa fa-save"></i> Guardar</button>
    ...
    <script>
        function modificar(id, ci, nombre, apellido, fecha_nacimiento, celular) {
            ...
            var modal = new bootstrap.Modal(document.getElementById('modal-modificar'));
            modal.show();
        }
    </script>
</body>
